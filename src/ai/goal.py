...
def _fallback_plan(weight_kg: float, exercise_level: str, temp_c: float, health: str) -> dict:
    """无 AI 或解析失败时的方案：用公式算目标 + 通用建议。"""
...
def compute_plan(
    ...
    """返回当日饮水方案字典：
        {goal_ml, note, rhythm, tips[], caution, needs_doctor, reminder_lines[]}
    """
    health = (health or "").strip()
    fallback = _fallback_plan(weight_kg, exercise_level, temp_c, health)
    ...
    system = (
        ...
        '"reminder_lines": ["亲切的提醒语，结合其健康状况", ...](3-5条，每条不超过30字)}'
    )
    user = (
        f"体重: {weight_kg} kg\n"
