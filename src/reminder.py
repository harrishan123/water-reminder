...
class AppService:
    ...
    def ensure_today_goal(self) -> int:
        """确保今天有目标值；若配置了固定目标则用固定值，否则按需计算。"""
        existing = self.storage.get_goal()
        if existing:
            return existing

        fixed = int(self.cfg.get("profile.daily_goal_ml", 0) or 0)
        if fixed > 0:
            self.storage.set_goal(fixed, note="固定目标")
            return fixed

        temp = self.current_temperature()
        weight = float(self.cfg.get("profile.weight_kg", 65))
        level = str(self.cfg.get("profile.exercise_level", "light"))
        goal, note = goal_mod.compute_daily_goal(self.ai, weight, level, temp or 0.0)
        self.storage.set_goal(goal, note=note)
        log.info("今日目标: %sml (%s)", goal, note)
        return goal

    def recompute_goal(self) -> int:
        """清空今日目标与气温缓存并重新计算(供托盘/网页「重算目标」调用)。"""
        self.storage.set_goal(0, day=date.today(), note="")
    ...
    def _fire_reminder(self) -> None:
        ...
        if self._paused or not self._within_active_hours():
            return
        self.last_reminder_text = random.choice(REMINDER_LINES)
